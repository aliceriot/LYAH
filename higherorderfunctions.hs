...
biggestNum :: (Integral a) => a
biggestNum = head [x | x <- [100000,99999..], mod x 3829 == 0]

-- or with filter

biggestNum2 :: (Integral a) => a
biggestNum2 = head (filter p [100000,99999..])
    where p x = mod x 3829 == 0

-- sum of all odd squares <10000

sumSquaresSmaller :: (Integral a) => a
...
collatzCount :: Int
collatzCount = length [x | x <- map collatzChain [1..100], length x > 15]
...
sqrtSum = length [x | x <- (takeWhile (<1000) (scanl1 (+) (map sqrt [1..])))] + 1
